...
- **No in-guest TLS.** `ssl`, `public_key`, and `asn1` are stubs that satisfy the dependency graph but provide no functions. Terminate TLS at the load balancer and serve plain HTTP in-guest. (For TLS to a *database*, a sidecar proxy works — see [`docs/DEPLOY.md`](docs/DEPLOY.md).) An `https:` listener starts and then `:undef`s at request time; `tyn-pack` warns when it detects one.
...
- **Wall clock is RTC-seeded, second-resolution, and drifts.** `CLOCK_REALTIME` is seeded from the hardware RTC at boot (real UTC — fine for `DateTime.utc_now`, logs, `Date` headers) but is second-resolution and drifts with the TSC over long uptime; no NTP/kvmclock sync. Monotonic time is exact.

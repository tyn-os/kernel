...
- **In-guest inbound TLS: available via rustls — UNREVIEWED pending audit.** HTTPS terminates *in-guest* through the `tyn_tls` rustls NIF + a `ThousandIsland` transport (config-only wire-in) — no plaintext hop to an edge terminator. Proven on Nitro (TLS 1.3, byte-exact). See [`docs/IN_GUEST_TLS.md`](docs/IN_GUEST_TLS.md) and [`examples/tls/`](examples/tls/). Caveats: the pure-Rust crypto surface is **not yet externally reviewed** (RNG-review-first); OTP's own `ssl`/`public_key`/`asn1` remain stubs (this path bypasses them); **outbound** (client-side) TLS and TLS-to-a-database are **not yet** in-guest — use a sidecar for DB TLS ([`docs/DEPLOY.md`](docs/DEPLOY.md)). Not a *confined* TCB yet (BEAM and kernel share ring 0).

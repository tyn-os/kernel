...
impl<H: Hal, const SIZE: usize> VirtQueue<H, SIZE> {
    ...
    /// Returns diagnostic information about the queue state.
    /// Returns (queue_idx, avail_ptr, avail_idx_local, avail_idx_ring, used_ptr, used_idx_ring)
    pub fn diag(&self) -> (u16, u64, u16, u16, u64, u16) {
        let avail_ptr = self.avail.as_ptr() as u64;
        let avail_idx_ring = unsafe {
            (*self.avail.as_ptr()).idx.load(Ordering::Relaxed)
        };
        let used_ptr = self.used.as_ptr() as u64;
        let used_idx_ring = unsafe {
            (*self.used.as_ptr()).idx.load(Ordering::Relaxed)
        };
        (self.queue_idx, avail_ptr, self.avail_idx, avail_idx_ring, used_ptr, used_idx_ring)
    }

...
extern "C" fn main(_mbi: *const u8) -> ! {
    ...
    unsafe {
        ...
        let args: &[&[u8]] = &[
            ...
            b"-noshell\0",
            b"-noinput\0",
            b"-eval\0", b"erlang:display({otp,erlang:system_info(otp_release)}).\0",
            b"-eval\0", b"erlang:display({procs,length(erlang:processes())}).\0",
            b"-eval\0", b"erlang:display({math,1+1}).\0",
            b"-eval\0", b"erlang:display({spawn_test,spawn(fun()->erlang:display(hello_from_spawn)end)}).\0",
        ];
